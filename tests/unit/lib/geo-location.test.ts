...
describe('getGeoLocation', () => {
  test('returns geolocation data from the API if `mode: "cache"`', async () => {
    ...
    // @ts-expect-error TS(2345) FIXME: Argument of type '{ mode: string; state: { get(): ... Remove this comment to see the full error message
  ...
  test('returns geolocation data from cache if data is younger than TTL', async () => {
    ...
    // @ts-expect-error TS(2345) FIXME: Argument of type '{ mode: string; state: { get(key... Remove this comment to see the full error message
    const geo = await getGeoLocation({ mode: 'cache', state: mockState })
  ...
  test('returns geolocation data from cache, even if older than TTL, if the application is offline', async () => {
    ...
    // @ts-expect-error TS(2345) FIXME: Argument of type '{ mode: string; offline: boolean... Remove this comment to see the full error message
