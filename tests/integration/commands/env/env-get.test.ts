...
describe('env:get command', () => {
  setupFixtureTests('empty-project', { mockApi: { routes } }, () => {
    ...
    test.concurrent<FixtureTestContext>('should return the value for the given context when present', async ({
      fixture,
    }) => {
    ...
    test.concurrent<FixtureTestContext>('should not return the value for the given context when not present', async ({
      fixture,
    }) => {
      const cliResponse = await fixture.callCli(['env:get', '--json', 'EXISTING_VAR', '--context', 'deploy-preview'], {
        offline: false,
        parseJson: true,
      })
    ...
    test.concurrent<FixtureTestContext>('should find the value for the given scope when present', async ({
      fixture,
    }) => {
    ...
    test.concurrent<FixtureTestContext>('should not find the value for the given scope when not present', async ({
      fixture,
    }) => {
