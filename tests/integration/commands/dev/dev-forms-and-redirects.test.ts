  ...
  test('should use existing file when rule contains file extension and force=false', async (t) => {
    await withSiteBuilder(t, async (builder) => {
      ...
      await withDevServer({ cwd: builder.directory }, async (server) => {
        ...
        // @ts-expect-error TS(2339) FIXME: Property 'location' does not exist on type 'Header... Remove this comment to see the full error message
    ...
    await withSiteBuilder(t, async (builder) => {
      ...
      t.expect(() =>
        withDevServer(
          ...
          // @ts-expect-error TS(2345) FIXME: Argument of type '{ message: RegExp; }' is not ass... Remove this comment to see the full error message
