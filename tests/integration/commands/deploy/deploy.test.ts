...
describe.skipIf(process.env.NETLIFY_TEST_DISABLE_LIVE === 'true').concurrent('commands/deploy', () => {
  ...
  test.only('runs build command before deploy by default', async (t) => {
    await withSiteBuilder(t, async (builder) => {
      const content = '<h1>⊂◉‿◉つ</h1>'
      builder
        .withContentFile({
  ...
  test.only('does not run build command before deploy when --build=false flag is passed', async (t) => {
    await withSiteBuilder(t, async (builder) => {
      const content = '<h1>⊂◉‿◉つ</h1>'
      builder
        .withContentFile({
      ...
      t.expect(output.includes('Netlify Build completed in')).toBe(false)
      const [, deployId] = output.match(/DEPLOY_ID: (\w+)/) ?? []
      const [, deployURL] = output.match(/DEPLOY_URL: (.+)/) ?? []
