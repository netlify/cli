...
function withPathWarning(path, message) {
  console.log("\n");
  console.log(chalk.bold("Warning: ") + message);
  console.log("\n");

  return fn.call(inquirer.prompt, {name: "continue", message: "Are you sure you want to continue?", type: "confirm"}).then(function(result) {
    if (result.continue) {
      return path;
    }
    process.exit(1);
    ...
    if (!config.existing && path == process.cwd()) {
      for (var file in fileChecksCWD) {
        if (fs.existsSync(file)) {
          return withPathWarning(path, fileChecksCWD[file]);
        }
      }
    }
    return when.resolve(path);
