...
exports.init = function(config) {
  git.getRepoDetails().then(function(repo) {
    fn.call(inquirer.prompt, [
      ...
      confirm.withConfirmation({msg: "Looking good?"}).then(function() {
        console.log("Preparing deploy key");
        config.client.createDeployKey({}).then(function(key) {
          console.log(
            chalk.bold("\nGive this Netlify SSH public key access to your repo:\n\n") +
            key.public_key + "\n\n"
          );
          confirm.withConfirmation({msg: "I've installed the deploy key", default: "yes"}).then(function() {
            config.client.createSite({
              ...
              confirm.withConfirmation({msg: "I've installed the webhook"}).then(function() {
