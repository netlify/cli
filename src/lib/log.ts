...
const RED_BACKGROUND = picocolors.red('-background')
const [PRO, BUSINESS, ENTERPRISE] = ['Pro', 'Business', 'Enterprise'].map((plan) => picocolors.magenta(plan))
...
`
export const MISSING_AWS_SDK_WARNING = `A function has thrown an error due to a missing dependency: ${picocolors.yellow(
  'aws-sdk',
)}.
You should add this module to the project's dependencies, using your package manager of choice:

${picocolors.yellow('npm install aws-sdk --save')} or ${picocolors.yellow('yarn add aws-sdk')}
