...
export const promptEditorHelper = async ({ NETLIFYDEVLOG, config, log, picocolors, repositoryRoot, state }) => {
  ...
  if (!confirm) {
    log(
      `${NETLIFYDEVLOG} You can start this configuration manually by running ${picocolors.magenta(
        picocolors.bold('netlify recipes vscode'),
      )}.`,
    )
