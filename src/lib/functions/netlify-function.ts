...
export default class NetlifyFunction<BuildResult extends BaseBuildResult> {
  ...
  public buildData?: MappedOmit<BuildResult, 'includedFiles' | 'schedule' | 'srcFiles'>
  ...
  async build({ cache }: { cache?: BuildCommandCache<Record<string, unknown>> }) {
    ...
    try {
      ...
      this.buildData = (buildData satisfies Omit<BuildResult, 'includedFiles' | 'schedule' | 'srcFiles'>) as unknown as MappedOmit<BuildResult, 'includedFiles' | 'schedule' | 'srcFiles'>
