...
export default class NetlifyFunction<BuildResult extends BaseBuildResult = BaseBuildResult> {
  private readonly blobsContext: BlobsContextWithEdgeAccess
  private readonly config: PatchedConfig
  ...
  private buildQueue?: undefined | Promise<BuildResult>
  public buildData?: Omit<BuildResult, 'includedFiles' | 'schedule' | 'srcFiles'>
  ...
  async build({ cache }: { cache?: BuildCommandCache<BuildResult> }) {
  ...
  async getBuildData(): Promise<NonNullable<typeof this.buildData>> {
    await this.buildQueue

    // XXX make Nathan happy
