...
import { chalk, getTerminalLink, NETLIFYDEVERR, NETLIFYDEVLOG, NETLIFYDEVWARN, warn, watchDebounced, } from '../../utils/command-helpers.js';
...
export class FunctionsRegistry {
    ...
    static logEvent(event, { func, warnings = [] }) {
        ...
        if (event === 'buildError') {
            NetlifyLog.info(`${NETLIFYDEVERR} ${chalk.red('Failed to load')} function ${chalk.yellow(func?.displayName)}: ${func?.buildError?.message}`);
        }
        if (event === 'extracted') {
            NetlifyLog.info(`${NETLIFYDEVLOG} ${chalk.green('Extracted')} function ${chalk.yellow(func?.displayName)} from ${func?.mainFile}.`);
            return;
        }
        if (event === 'loaded') {
        ...
        if (event === 'missing-types-package') {
            NetlifyLog.info(`${NETLIFYDEVWARN} For a better experience with TypeScript functions, consider installing the ${chalk.underline(TYPES_PACKAGE)} package. Refer to https://ntl-fyi/function-types for more information.`);
        }
        if (event === 'reloaded') {
            const icon = warningsText ? NETLIFYDEVWARN : NETLIFYDEVLOG;
            const color = warningsText ? chalk.yellow : chalk.green;
