...
export const invokeFunction = async <BuildResult extends JsBuildResult>({
  context,
  environment,
  event,
  func,
  timeout,
}: Parameters<InvokeFunction<BuildResult>>[0]) => {
  ...
  const workerData = {
    ...
    entryFilePath: pathToFileURL(
      // XXX(serhalp) -- @ndhoule plzhelp this only happens to work because `typeof` refines the `unknown`, but it
      // shouldn't be unknown :(((
      'buildPath' in buildData && typeof buildData.buildPath === 'string' && buildData.buildPath
        ? buildData.buildPath
        : func.mainFile,
    ).href,
