...
export type ZisiBuildResult = BaseBuildResult & FunctionResult
...
const buildFunction = async ({
  ...
  cache: BuildCommandCache<ZisiBuildResult>
  config: NormalizedFunctionsConfig
  directory?: string | undefined
  featureFlags: FeatureFlags
  func: NetlifyFunction<ZisiBuildResult>
...
export const parseFunctionForMetadata = async ({
  config,
  mainFile,
  projectRoot,
}: {
...
const netlifyConfigToZisiConfig = ({ config, projectRoot }: { config: NetlifyConfig; projectRoot: string }) =>
...
export default async function detectZisiBuilder({
  ...
  const build = async ({ cache = {} }: { cache: BuildCommandCache<ZisiBuildResult> }) =>
    buildFunction({
      cache,
