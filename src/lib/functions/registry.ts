...
import runtimes, { type Runtime, type BaseBuildResult } from './runtimes/index.js'
  ...
  async scan(relativeDirs: (string | undefined)[]) {
    ...
    const addedFunctions = await Promise.all(
      ...
      functions.reverse().map(async ({ displayName, mainFile, name, runtime: runtimeName }) => {
        ...
        type ExtractRuntimeType<R> = R extends Runtime<infer T> ? T : never

        const func = new NetlifyFunction<ExtractRuntimeType<typeof runtime>>({
          ...
          // @ts-expect-error(serhalp) -- @ndhoule plzfix ayy lmao
          // I think TS needs to know that a given instance of `runtime` at this point will have a refined
          // type of only one of the runtime types in the union, and this type is consistent between the
          // `NetlifyFunction` and the `runtime`. But... how do?
          runtime,
