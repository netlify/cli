...
export const envList = async (options: OptionValues, command: BaseCommand) => {
  ...
  const forSite = `for site ${picocolors.green(siteInfo.name)}`
  const contextType = AVAILABLE_CONTEXTS.includes(context) ? 'context' : 'branch'
  const withContext = `in the ${picocolors.magenta(options.context)} ${contextType}`
  const withScope = scope === 'any' ? '' : `and ${picocolors.yellow(options.scope)} scope`
  ...
  if (showValues) {
    ...
    log(`${picocolors.cyan('?')} Show values? ${picocolors.cyan('Yes')}`)
  }
}
