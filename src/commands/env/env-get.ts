...
export const envGet = async (name: string, options: OptionValues, command: BaseCommand) => {
  ...
  if (!value) {
    ...
    const withContext = `in the ${picocolors.magenta(context)} ${contextType}`
    const withScope = scope === 'any' ? '' : ` and the ${picocolors.magenta(scope)} scope`
    log(`No value set ${withContext}${withScope} for environment variable ${picocolors.yellow(name)}`)
    return false
  }
