    ...
    switch (linkType) {
        ...
        case SITE_NAME_PROMPT: {
            ...
            if (!matchingSites || matchingSites.length === 0) {
                ...
                outro({
                    message: `Run ${chalk.cyanBright('netlify link')} again to try a new search, or run ${chalk.cyanBright('netlify sites:create')} to create a site.`,
                    exit: true,
                });
            }
            if (matchingSites.length > 1) {
        ...
        case SITE_LIST_PROMPT: {
            ...
            if (!sites || sites.length === 0) {
                NetlifyLog.error(`You don't have any sites yet. Run ${chalk.cyanBright('netlify sites:create')} to create a site.`);
            }
            loading.stop(`Found ${sites.length} sites!`);
            const selectedSite = await select({
                message: 'Which site do you want to link?',
                maxItems: 7,
                options: sites.map((matchingSite) => ({
                    label: matchingSite.name,
                    value: matchingSite,
                })),
            });
