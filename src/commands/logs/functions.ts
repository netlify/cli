...
import { picocolors, log } from '../../utils/command-helpers.js'
...
function getLog(logData: { level: string; message: string }) {
  ...
  switch (logData.level) {
    case LOG_LEVELS.INFO:
      logString += picocolors.blueBright(logData.level)
      break
    case LOG_LEVELS.WARN:
      logString += picocolors.yellowBright(logData.level)
      break
    case LOG_LEVELS.ERROR:
      logString += picocolors.redBright(logData.level)
      break
    default:
