...
const combineConsoleMessages = (message, optionalParams) => {
    if (!message)
        return '';
    if (optionalParams && optionalParams.length > 0) {
        // Replace all %s, %d, %i, %f, %o with the next optional param
        let formattedMessage = message.replace(/%[sdifo]/g, (match) => {
...
const transportLogsToNetlifyLog = () => {
    ...
    console.log = (message, ...optionalParams) => message && NetlifyLog.info(combineConsoleMessages(message, optionalParams));
    console.warn = (message, ...optionalParams) => message && NetlifyLog.warn(combineConsoleMessages(message, optionalParams));
    console.error = (message, ...optionalParams) => message && NetlifyLog.error(combineConsoleMessages(message, optionalParams));
    console.info = (message, ...optionalParams) => message && NetlifyLog.info(combineConsoleMessages(message, optionalParams));
};
/**
