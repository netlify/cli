...
import { picocolors, error, exit, getToken, log, logJson, warn, APIError } from '../../utils/command-helpers.js'
...
────────────────────┘`)
  log(
    prettyjson.render({
      ...
      'Admin URL': picocolors.magentaBright(siteInfo.admin_url),
      'Site URL': picocolors.cyanBright(siteInfo.ssl_url || siteInfo.url),
      'Site Id': picocolors.yellowBright(siteInfo.id),
    }),
  )
  log()
}
