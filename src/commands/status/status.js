...
export const status = async (options, command) => {
    ...
    try {
        ...
        if (error_.status === 401) {
            NetlifyLog.error('Your session has expired. Please try to re-authenticate by running `netlify logout` and `netlify login`.');
        }
    }
    ...
    NetlifyLog.message(prettyjson.render({
