...
import { picocolors, error as failAndExit, log, warn } from '../command-helpers.js'

import { getRecommendPlugins, getUIPlugins } from './plugins.js'

const formatTitle = (title: string) => picocolors.cyan(title)
...
export const formatErrorMessage = ({ error, message }) => {
  ...
  return `${message} with error: ${picocolors.red(errorMessage)}`
}
