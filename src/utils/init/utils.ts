...
export const updateSite = async ({
  ...
  api: NetlifyAPI
  options: Parameters<typeof api.updateSite>[0]['body']
  siteId: string
}) => {
  try {
...
export const setupSite = async ({
  ...
  pluginsToInstall: Array<{ package: string }>
  // @ts-expect-error(serhalp) -- @ndhoule plzfix ayy lmao
  repo: NonNullable<NonNullable<Parameters<typeof api.updateSite>[0]['body']>['repo']>
  siteId: string
}) => {
