 ...
 */
export const readRepoURL = async function (url: string) {
...
const getRepoURLContents = async function (
  repoHost: string,
  ownerAndRepo: string,
  contentsPath: string,
): Promise<unknown> {
...
export const validateRepoURL = function (url: string): typeof GITHUB | null {
  // eslint-disable-next-line n/no-deprecated-api -- TODO: use `url.URL()` instead
  const parsedURL = URL.parse(url)
...
export const parseRepoURL = function (repoHost: string, url: { path: string | null }) {
