...
const handleStaticServer = async ({ devConfig, flags, workingDir, }) => {
    ...
    if (flags.dir) {
        ...
        NetlifyLog.info(`Using simple static server because ${formatProperty('[dev.framework]')} was set to ${formatValue('#static')}`);
    }
    if (devConfig.targetPort) {
        NetlifyLog.info(`Ignoring ${formatProperty('targetPort')} setting since using a simple static server.${EOL} Use --staticServerPort or [dev.staticServerPort] to configure the static server port`);
    }
