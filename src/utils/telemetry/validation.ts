...
export default function isValidEventName(eventName: string, config: EventConfig): boolean {
  ...
  if (!validProject.includes(project)) {
    const error = `> "${project}" is invalid tracking project. Must be one of ${JSON.stringify(validProject)}`
    return formattingWarning(eventName, error)
  }
  // validate object name
  if (!validObjects.includes(object)) {
    const error = `> "${object}" is invalid tracking object. Must be one of ${JSON.stringify(validObjects)}`
    return formattingWarning(eventName, error)
  }
  return true
}
...
const formattingWarning = function (eventName: string, errorMsg?: string): false {
  log('-----------------------------')
  log('Tracking Error:')
